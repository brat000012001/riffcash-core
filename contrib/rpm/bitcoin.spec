...
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 19764
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 19765
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 119764
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 119765
...
if [ $1 -eq 0 ]; then
	if [ `%{_sbindir}/sestatus |grep -c "disabled"` -eq 0 ]; then
	%{_sbindir}/semanage port -d -p tcp 19764
	%{_sbindir}/semanage port -d -p tcp 19765
	%{_sbindir}/semanage port -d -p tcp 119764
	%{_sbindir}/semanage port -d -p tcp 119765
	for selinuxvariant in %{selinux_variants}; do
