...
*Setup instructions for a Gitian build of Riffcash Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Riffcash
...
- [Building Riffcash Core](#building-riffcash-core)
...
Clone the git repositories for riffcash and Gitian.
...
in the riffcash repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building Riffcash Core
----------------

To build Riffcash Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the riffcash repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/riffcash/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/riffcash-project/riffcash
...
For example:
```bash
URL=https://github.com/thrasher-/riffcash.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit riffcash=${COMMIT} --url riffcash=${URL} ../riffcash/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit riffcash=${COMMIT} --url riffcash=${URL} ../riffcash/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit riffcash=${COMMIT} --url riffcash=${URL} ../riffcash/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the riffcash git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../riffcash/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/riffcash
SIGPATH=/some/root/path/riffcash-detached-sigs

./bin/gbuild --url riffcash=${BTCPATH},signature=${SIGPATH} ../riffcash/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/riffcash-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/riffcash-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/riffcash-osx-build.assert
```
...
[riffcash-project/gitian.sigs.rfc](https://github.com/riffcash-project/gitian.sigs.rfc/) repository, or if that's not possible create a pull
