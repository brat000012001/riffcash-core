...
Run with the -testnet option to run with "play riffcashs" on the test network, if you
...
Riffcash Core is a multithreaded application, and deadlocks or other multithreading bugs
...
- ThreadSocketHandler : Sends/Receives data from peers on port 19765.
...
- ThreadRPCServer : Remote procedure call handler, listens on port 19764 for connections and services them.

- BitcoinMiner : Generates riffcashs (if wallet is enabled).
...
However, in open source software such as Riffcash Core, where everyone uses
...
pay attention to for reviewers of Riffcash Core code.

General Riffcash Core
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: Riffcash Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
Some of these are maintained by active developers of Riffcash Core, in which case changes should probably go
directly upstream without being PRed directly against the project.  They will be merged back in the next
subtree merge.

Others are external projects without a tight relationship with our project.  Changes to these should also
be sent upstream but bugfixes may also be prudent to PR against Riffcash Core so that they can be integrated
...
- When looking at other's pull requests, it may make sense to add the following section to your `.git/config`
  file:
        ...
        [remote "upstream-pull"]
                ...
                url = git@github.com:riffcash-project/riffcash.git
